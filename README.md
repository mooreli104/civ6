...
40 games per agent on held-out seeds (identical boards for every agent), score =
accumulated science + culture + faith after 40 turns:

| agent | mean score | median | best | win rate vs heuristic |
|---|---:|---:|---:|---:|
| **trained policy** (greedy) | **167.6** | 164.0 | 329 | **70%** |
| trained policy (T=0.4) | 165.2 | 151.5 | 386 | 62% |
| heuristic baseline | 146.6 | 157.5 | 249 | — |
| scripted (the package's own example strategy) | 18.0 | 19.0 | 49 | 0% |
| random | 2.3 | 1.5 | 6 | 0% |

The policy is trained purely by imitating the best games the heuristic produced
(top 15% of 1200), and comes out ahead of the teacher it learned from. Full
tables, training curves and self-play rounds live in `reports/` and are
regenerated by every run (`src/make_report.py` collates them into
`reports/RESULTS.md`).
